...
[Phase 2.5: 对标账号 — 强烈建议（cold-start 必须问，已发用户可选）]
  ↓
[Phase 3: 创建脚手架（含 scripts/ + videos/ + samples/ + approved_samples/ 空目录 + 模板文件含 benchmark.md）]
  ↓
...
4. **五个目录**：`scripts/` + `predictions/` + `videos/` + `samples/` + `approved_samples/`（都加 `.gitkeep`；`approved_samples/README.md` 另行创建）
   ```
   "正在创建四个目录：
    ...
    videos/       — 拍后的工作目录（cheat-shoot 创建子目录）
     ...
     approved_samples/ — 你明确认可的最终脚本（cheat-approve 管理）
...
4.8. **`voice_profile.md` + `approved_samples/`**
   ```
   "正在创建 voice_profile.md 和 approved_samples/ ——你的个人创作记忆。

    只有你明确认可的成稿或成片脚本，才会进入这里。
    voice_profile.md 保存从样本提炼的语气、节奏和词汇；
    approved_samples/ 保留完整原文，方便以后追溯。
    没有认可的稿子不会自动学习，播放数据也不会被当成语气信号。"
   ```
   - 复制 `cheat-on-content/templates/voice_profile.template.md` → `<user-repo>/voice_profile.md`
   - 创建 `<user-repo>/approved_samples/README.md`（如已存在不覆盖）
...
✅ 认可一条稿子 → "沉淀这条 scripts/<...>.md"（只把你明确认可的内容加入创作记忆）
...
- `cheat-approve` 只收录用户明确认可的样本；`cheat-seed` 后续写稿时优先读取 `voice_profile.md` + `approved_samples/`
