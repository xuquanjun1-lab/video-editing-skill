...
如果用户在复盘时明确说“这条文案 / 这个视频就是我认可的风格”，把“表现数据复盘”和“创作记忆收录”分开处理：先完成本 skill 的实绩记录，再路由到 `/cheat-approve` 保存用户指向的脚本或转写。仅有高播放 / 高点赞而没有明确风格认可时，不调用 `/cheat-approve`。
...
- 创作记忆：只有用户明确认可的脚本才由 `/cheat-approve` 写入 `approved_samples/`；复盘数据本身不自动变成语气样本
