...
- `沉淀这条 scripts/foo.md` / `这就是我的语气` → 读取并执行 `skills/cheat-approve/SKILL.md`
...
| "认可这条" / "沉淀这条" / "这是我的语气" / "以后按这个来" | `/cheat-approve` | 已 init；必须有明确指向的最终稿或成片脚本 |
...
├── approved_samples/                  # 用户明确认可的最终脚本（创作参考，不进盲预测）
│   └── README.md
...
│   ├── cheat-approve/SKILL.md         # ✅ 用户认可样本沉淀（语气 + 写法）
...
## 用户偏好的完整拍摄脚本输出规范（追加约定）

本节为用户偏好的补充交付规范，只追加在原有规则之后，不覆盖、不删除、不修改前文任何规则。

当用户提供一段自己想说的脚本，并要求改写、整理或生成拍摄脚本时，默认直接输出一份可以拿去拍摄的完整脚本，而不是只给一段润色后的文案。除非用户明确要求只要文案，否则必须把口播内容转换为可执行的分镜方案。
...
1. **第一段：真人正面口播**
   - 先讲真实经历、观察或问题。
   - 用用户自己的表达作为基础，不强行改成广告腔或新闻腔。
   - 第一段完整讲完后，再进入产品、工具或案例展示。

2. **第二段：小窗录屏 + 旁白**
   - 录屏画面占主要区域，真人以小窗形式保留在角落。
   - 旁白配合实际操作，按观众能理解的顺序介绍功能。
   - 不要求展示所有按钮，只展示能支撑主线的 3—5 个核心功能。
   - 每个功能必须写明具体录屏动作、停留时长和对应话术。

3. **第三段：回到真人继续讲解**
   - 展示结束后回到真人正面口播。
   - 解释适用人群、真实使用场景、能解决的问题和使用边界。
   - 不做绝对化承诺，不写“完全替代”“保证赚钱”“一定省钱”等未经验证的结论。

4. **结尾：轻量行动钩子**
   - 结合视频主题给出一个具体动作，例如评论关键词、私信关键词或关注后看后续更新。
   - 钩子应与正文有直接关系，不用生硬的营销话术。
   - 可以预告后续测试、功能更新、真实案例或效果对比，形成自然的追更理由。

### 必须交付的内容

完整拍摄脚本至少包含以下部分：

- **视频总览**：建议总时长、画幅、口播语速、整体情绪和镜头比例。
- **拍摄准备清单**：明确需要提前准备的图片、录屏页面、演示数据或道具，不能把“找素材”留给用户自行判断。
- **逐镜头分镜表**：每个镜头写明时间码或时长、景别/画面、人物动作、屏幕操作、准确台词、字幕重点、转场和声音处理。
- **录屏插入位置**：明确从哪一句切入录屏、录屏展示什么、何时切回真人，不让用户自行计算位置。
- **整合版逐字稿**：把分镜中的所有口播台词按顺序合并，方便用户直接照着念或提词器拍摄。
- **拍摄执行提醒**：包括语气、停顿、视线、鼠标速度、录屏节奏和结尾停留画面等实际操作细节。

### 时长与口播计算规则

- 默认按普通中文口播速度估算时长，约每秒 3—4.5 个汉字，并为停顿、切换画面和观众理解预留时间。
- 每个镜头直接给出明确的起止时间，例如“0:23—0:32”，不让用户自己换算秒数。
- 口播内容过长时，优先删掉重复解释和空话，不牺牲真实经历、核心观点和必要边界说明。
- 录屏片段以“一个功能一个动作”为单位，通常 3—8 秒；避免连续快速点击造成观看压力。

### 语言与改写边界

- 保留用户原稿的事实、经历、判断和说话习惯；允许整理重复、补齐逻辑和调整顺序。
- 口吻要像真实创作者在分享自己的制作过程，使用短句、自然停顿和生活化连接词。
- 先给具体现象或结果，再解释原因和做法；围绕一个主承诺推进，不平均堆砌产品功能。
- 不凭空增加数据、客户数量、收益、效果或用户反馈；原稿没有的信息只能用谨慎表达补充。
- 产品展示要说明“现在能看到什么”和“实际能解决什么”，同时说明尚未验证或不能替代的部分。
...
三、完整分镜表
| 时间 | 画面/景别 | 人物动作或屏幕操作 | 口播/旁白 | 字幕、声音、转场 |

四、整合版逐字稿
（按拍摄顺序合并全部台词）

五、结尾钩子与执行提醒
（明确私信/评论/关注动作，以及最后一帧停留内容）
```
...
这个协议把“生成得像我”变成可追溯的创作记忆，但不把未经确认的草稿误当成用户风格。

- **只有明确认可才学习**：用户说“这条我认可”“这就是我的语气”“以后按这个来”“沉淀这条”等，才调用 `/cheat-approve`。
- **样本与规则分开**：完整原文保存到项目的 `approved_samples/`；提炼出的语气、节奏、词汇和结构写入 `voice_profile.md`；可复用结构写入 `script_patterns.md`。
- **拒绝不入库**：用户说“不对”“效果不好”“重写”，或只给出播放量 / 点赞量，不会自动学习。
- **单样本不封神**：一个认可样本只标为“用户认可、待验证”；重复出现并得到更多认可后，才提升为稳定 pattern。
- **创作记忆不污染预测**：`voice_profile.md` 和 `approved_samples/` 只供选题和写稿使用，不喂给 `cheat-score-blind`，也不写入 `rubric_notes.md`。真实表现仍通过 `cheat-retro` 进入校准链路。
- **未来写稿优先级**：`cheat-seed` 先读用户明确指定的偏好和 active 的认可样本，再读用户 pattern、历史拍摄稿、对标账号和通用 rubric。

如果用户要求撤销某个样本，标记为 `revoked` 并让后续生成忽略；除非用户明确确认准确路径，否则不做不可恢复删除。
